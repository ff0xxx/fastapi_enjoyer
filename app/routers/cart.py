...
async def _get_cart_item(db: AsyncSession, user_id: int, product_id: int):
    cart_item = await db.scalar(select(CartItemModel)
...
async def get_cart(db: AsyncSession = Depends(get_async_db), 
    ...
    res = await db.scalars(select(CartItemModel)
                           .options(selectinload(CartItemModel.product))
    ...
    total_price = sum([item.quantity * item.product.price for item in items], Decimal("0"))
...
async def add_item_to_cart(payload: CartItemCreate,
    ...
    await db.commit()
    # refresh(cart_item) обновит только атрибуты самого CartItemModel, но не загрузит связанный product, so
